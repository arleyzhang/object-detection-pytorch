...
__all__ = ['drn_d_22', 'drn_d_39', 'drn_d_24']
...
class DRN(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000,
        ...
        self.layer6 = None if layers[5] == 0 else \
            self._make_layer(block, channels[5], layers[5], stride=1, dilation=4,
                             new_level=False)
        ...
        if arch == 'C':
            ...
            self.layer8 = None if layers[7] == 0 else \
                self._make_conv_layers(
                    channels[7], layers[7], stride=1, dilation=1)
            self.layer9 = None if layers[8] == 0 else \
                self._make_conv_layers(
                    channels[8], layers[8], stride=1, dilation=1)

            self.layer10 = None if layers[9] == 0 else \
                self._make_conv_layers(
                    channels[9], layers[9], stride=1, dilation=1)
            self.layer11 = None if layers[10] == 0 else \
                self._make_conv_layers(
                    channels[10], layers[10], stride=1, padding=0, dilation=1, batchnorm=False)
    ...
    def _make_conv_layers(self, channels, convs, stride=1, padding=None, dilation=1, batchnorm=True):
        modules = []
...
drn_channels = {
    'drn_d_22': [16, 32, 64, 128, 256, 512, 512, 512, 512, 256, 256],
...
# v25
def drn_d_22(**kwargs):
    model = DRN(BasicBlock, [1, 1, 2, 2, 2, 2, 1, 1, 1, 2, 2],
                channels=drn_channels['drn_d_22'], out_middle=False, arch='D', **kwargs)
    return model

# # v26
# def drn_d_24(**kwargs):
#     model = DRN(BasicBlock, [1, 1, 2, 2, 2, 2, 1, 1, 1, 0, 0],
#                 channels=drn_channels['drn_d_22'], out_middle=False, arch='D', **kwargs)
#     return model

# v30
def drn_d_24(**kwargs):
    model = DRN(BasicBlock, [1, 1, 2, 2, 2, 2, 2, 2, 2, 0, 0],
                channels=drn_channels['drn_d_22'], out_middle=False, arch='D', **kwargs)
    return model
