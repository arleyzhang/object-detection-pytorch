...
class SSD(nn.Module):
    ...
    def __init__(self, phase, cfg, base):
        ...
        head = multibox(self, predict_conv_source['vgg16'], cfg.MODEL.NUM_PRIOR, cfg.MODEL.NUM_CLASSES)
...
def get_predict_layers(module, predict_source_name):
    predict_layers = []
    for name, sub_module in module.named_modules():
        if name in predict_source_name:
            predict_layers.append(sub_module)
    return predict_layers


def multibox(module, predict_source_name, num_priors, num_classes):
    loc_layers = []
    conf_layers = []
    predict_source = get_predict_layers(module, predict_source_name)
...
predict_conv_source = {
    'vgg16': ['base.21', 'base.33', 'extras.2', 'extras.6', 'extras.10', 'extras.14'],
    'drn_d_23': []
}
