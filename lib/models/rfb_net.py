...
class RFBNet(nn.Module):
    ...
    Args:
        phase: (string) Can be "eval" or "train"
    ...
    def __init__(self, phase, cfg, base):
        ...
        self.size = cfg.MODEL.IMAGE_SIZE[-1]
        size = cfg.MODEL.IMAGE_SIZE[-1]
        ...
        if size == 300:
            ...
            return
        _,extras,head=multibox(size, vgg(base_config[str(size)], 3),
                 add_extras(size, extras_config[str(size)], 512),
                 mbox[str(size)], num_classes)

        # vgg network
        if isinstance(base, nn.ModuleList):
            self.base = nn.ModuleList(base)
        else:
            self.base = base
        # conv_4
        self.Norm = BasicRFB_a(256,256,stride = 1,scale=1.0)
...
def add_extras(size, cfg, i, batch_norm=False):
    ...
    if size == 512:
        ...
        layers += [BasicConv(256,128,kernel_size=1,stride=1)]
        layers += [BasicConv(128,256,kernel_size=3,stride=1,bn=False)]
    else:
